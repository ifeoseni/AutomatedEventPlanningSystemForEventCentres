...
<div class="card shadow mb-4">
  ...
  <div class="card-body">
    ...
    <div class="table-responsive">
      <?php
       echo "<h3 class='text-primary text-bold'> This budget is updated as of ". date('l d M Y')."</h3>";
      ...
      <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
        <thead>
          <tr class="text-white bg-info">
            ...
            <th>Date <i class="fa fa-calendar text-danger "></i>  This Budget Was Made?Last Edited</th>
            <th>Edit Budget Item</th>
            <th>Delete Budget Item</th>
              ...
              <tr>
                  ...
                  <?php
                  	if ($row['estimate_price'] > $row['amount_spent']) {
                  		echo "<td class='text-warning'>".$row['estimate_price']."</td>";
                  		echo "<td class='text-success'>".$row['amount_spent']."</td>";
                  	}else if($row['estimate_price'] == $row['amount_spent']){
                      echo "<td class='text-dark'>".$row['estimate_price']."</td>";
                      echo "<td class='text-dark'>".$row['amount_spent']."</td>";
                    }
                    else{
                  		echo "<td class='text-warning'>".$row['estimate_price']."</td>";
        ...
        <tfoot class="text-dark font-weight-bold">
        	<th> Total </th><?php 
        	$see=  mysqli_query($connection,"SELECT SUM(estimate_price)  FROM budget WHERE username='$username'");
        	$tSpend=  mysqli_query($connection,"SELECT SUM(amount_spent)  FROM budget WHERE username='$username'");
        	$pIncome=  mysqli_query($connection,"SELECT SUM(proposed_income)  FROM budget WHERE username='$username'");
        	echo "<th></th>";
 ...
 <span> Estimate Price Is Greater Than Cost Price <canvas class="bg-warning" ></canvas> </span>
  <span> Cost Price Is Less Than Estimate Price <canvas class="bg-success" ></canvas> </span><br/>
<span> Estimate Price Is Less Than Cost Price <canvas class="bg-warning" ></canvas> </span>
  <span> Cost Price Is Greater Than Estimate Price <canvas class="bg-danger" ></canvas> </span><br/>
  <span> Estimate Price Is Equal To Cost Price <canvas class="bg-dark" ></canvas> </span>
  <span> Cost Price Is Equal To Estimate Price <canvas class="bg-dark" ></canvas> </span><br/>
</h3>
       <?php
            } 
          else
          {
            echo "<p class='text-dark text-center font-weight-bold'>You have not created any budget. Click add to create budget now</p>";
          } 
        ?>
